...
// TROCA 1: Importa Pool em vez de Client
...
// ... (a função mapCRMEventToFacebookEvent continua a mesma) ...
...
// TROCA 2: Cria um Pool em vez de um Client
const pool = new Pool({
...
// TROCA 3: A inicialização do banco usa o pool para executar as queries
const initializeDatabase = async () => {
    try {
        // Apenas para verificar, não precisa de connect() permanente
        const checkClient = await pool.connect();
        console.log('Conexão com o pool do banco de dados estabelecida.');
        checkClient.release(); // Libera o cliente de volta para o pool
...
        console.log('Tabela "leads" verificada/criada com sucesso.');
        ...
        // ... (o resto da lógica de adicionar colunas pode continuar usando pool.query)
...
// ... (A rota /importar continua a mesma) ...
app.get('/importar', (req, res) => { res.send(`...`) });


// TROCA 4: A rota /import-leads agora usa pool.query
...
            ON CONFLICT (facebook_lead_id) DO UPDATE SET email = EXCLUDED.email, phone = EXCLUDED.phone, first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, dob = EXCLUDED.dob, city = EXCLUDED.city, estado = EXCLUDED.estado, zip_code = EXCLUDED.zip_code;
        `;
        for (const lead of leadsToImport) {
            if (!lead || typeof lead !== 'object' || !lead.facebook_lead_id) continue;
            // AQUI ESTÁ A MUDANÇA: usa pool.query em vez de client.query
            await pool.query(queryText, [
                lead.facebook_lead_id, lead.email || null, (lead.phone || '').replace(/\D/g, ''), lead.first_name || null, lead.last_name || null, lead.dob || null, lead.city || null, lead.estado || null, lead.zip_code || null
            ]);
        }
...
// TROCA 5: A rota /webhook agora usa pool.query
app.post('/webhook', async (req, res) => {
    try {
        // ... (lógica inicial do webhook continua a mesma) ...
...
        const dbRow = result.rows[0];
        // ... (o resto da lógica para enviar para o Facebook continua a mesma) ...
        ...
        const eventData = { event_name: facebookEventName, event_time: Math.floor(Date.now() / 1000), action_source: 'system_generated', user_data: userData, custom_data: { lead_id: dbRow.facebook_lead_id, event_source: 'crm', lead_event_source: 'Your CRM' } };
